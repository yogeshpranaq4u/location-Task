...
class RecyclerAdapter(private val list: List<Article>, private val clickListeners: ClickListeners) :
    ...
    class MyViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindView(list: List<Article>, context: Context, clickListeners: ClickListeners) {
            title?.text = list[adapterPosition].title
            image?.let {
                Glide
                    .with(context)
                    .load(list[adapterPosition].urlToImage)
                    .into(it)
            }
